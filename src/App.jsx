...
const DateTime = ({ isDark }) => {
  ...
  return (
    <div className="flex flex-col gap-1 transition-colors duration-700">
      <p className="text-blue-600 font-[1000] text-3xl tracking-tighter uppercase italic">
    ...
    <div className="fixed inset-0 z-[9999] bg-[#050a18] flex flex-col items-center justify-center overflow-hidden font-sans">
      ...
      <div className="mt-12 flex flex-col items-center gap-2">
        <div className="flex gap-1">
            ...
            <div className="w-1 h-1 bg-blue-600 animate-bounce" style={{animationDelay: '0.1s'}}></div>
            <div className="w-1 h-1 bg-blue-600 animate-bounce" style={{animationDelay: '0.2s'}}></div>
        </div>
    ...
    <div className={`min-h-screen transition-colors duration-700 ${activeTheme.bg} ${activeTheme.text}`}>
            ...
            {currentTab === "schedule" ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {scheduleData[selectedDay]?.map((sub, i) => (
                  <div key={i} style={{ animationDelay: `${i * 0.05}s` }} className={`p-8 rounded-[2.5rem] border transition-all duration-500 relative animate-card-entrance ${today === selectedDay && i === activeSlot ? "bg-blue-600 border-transparent shadow-2xl scale-105 text-white z-10" : `${activeTheme.card} ${activeTheme.border}`}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Object.entries(courseInfo).filter(([_, d]) => !d.isNonFaculty).map(([course, data], i) => (
                  <div key={i} style={{ animationDelay: `${i * 0.05}s` }} className={`p-8 rounded-[2rem] border animate-card-entrance ${activeTheme.card} ${activeTheme.border}`}>
      ...
      <div className={`fixed inset-0 z-[2000] flex items-end transition-all duration-500 ${isInfoOpen ? 'bg-black/60 backdrop-blur-sm pointer-events-auto' : 'bg-transparent pointer-events-none'}`} onClick={() => setIsInfoOpen(false)}>
          <div className={`w-full max-w-2xl mx-auto p-10 rounded-t-[3rem] border-t transition-transform duration-500 ${activeTheme.card} ${activeTheme.border} ${isInfoOpen ? 'translate-y-0' : 'translate-y-full'}`} onClick={e => e.stopPropagation()}>
            <div className="w-12 h-1.5 bg-blue-600/20 rounded-full mx-auto mb-8" />
            <h4 className="font-black italic uppercase text-2xl mb-6 text-center">Contact me</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-6 rounded-3xl border border-white/10 bg-black/20 text-center"><p className="text-blue-600 text-[10px] font-black mb-1">DEV</p><p className="font-black italic text-xl">Thant Zaw</p></div>
              <div className="p-6 rounded-3xl border border-white/10 bg-black/20 text-center"><p className="text-blue-600 text-[10px] font-black mb-1">CALL</p><p className="font-black italic text-xl">09982268184</p></div>
            </div>
            <button onClick={() => setIsInfoOpen(false)} className="w-full mt-8 py-5 bg-blue-600 text-white font-black uppercase rounded-2xl active:scale-95 transition-transform">Close</button>
          </div>
      </div>
